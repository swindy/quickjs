...
namespace QuickJS
{
    public class ClearScriptEngine : IJavaScriptEngine, IDisposable
    {
        ...
        private EngineContext Context { get; }
        ...
        public ClearScriptEngine(EngineContext context, bool debug, bool awaitDebugger)
        {
            Context = context;
            ...
            var runtimeFlags = V8RuntimeFlags.EnableDynamicModuleImports |
                               (debug
                                   ? (V8RuntimeFlags.EnableDebugging | V8RuntimeFlags.EnableRemoteDebugging)
                                   : V8RuntimeFlags.None);
            ...
            JavascriptDocumentType documentType = JavascriptDocumentType.Script)
        {
            ...
            var document = remoteUrl != null || hasUri
                ? new DocumentInfo(remoteUrl ?? url)
                    { Category = category, Flags = flags, ContextCallback = DocumentContextCallback }
                : new DocumentInfo(fileName)
                    { Category = category, Flags = flags, ContextCallback = DocumentContextCallback };
            ...
            if (ShouldAwait && isMainFile)
            {
                ...
                var message =
                    "Debugger connection timed out after 10 seconds. You can uncheck AwaitDebugger if you are not planning to connect a debugger.";
        ...
        public class DocumentLoader : DefaultDocumentLoader
        {
            ...
            public DocumentLoader(EngineContext ctx)
            {
                Context = ctx;
            }

            public override Task<Document> LoadDocumentAsync(DocumentSettings settings, DocumentInfo? sourceInfo,
                string specifier, DocumentCategory category, DocumentContextCallback contextCallback)
            {
                if (!specifier.StartsWith("http"))
                {
                    specifier = Context.ResolvePath(specifier);
                    if (sourceInfo.HasValue)
                        sourceInfo = new DocumentInfo(new Uri(specifier))
                        {
                            Category = category,
